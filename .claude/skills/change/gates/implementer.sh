#!/usr/bin/env bash
# implementer.sh — Convention gate: G3 verdict check for implementer agents.
set -euo pipefail
...
if [[ ${#ERRORS[@]} -gt 0 ]]; then
  deny_errors "implementer gate blocked: " "Complete G3 gate (plan approval) before spawning implementer."
fi
